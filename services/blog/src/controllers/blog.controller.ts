...
export const getSingleBlog = TryCatch(async (req, res) => {
  ...
  const singleBlog = blog[0]; // Extract the first element

  // Check the extracted element instead of array length
  if (!singleBlog) {
    res.status(404).json({
  ...
  const { data } = await axios.get(
    // TypeScript now knows singleBlog is defined
    `${process.env.USER_SERVICE}/api/v1/user/${singleBlog.author}`,
  );

  const responseData = { blog: singleBlog, author: data };
...
export const deleteComment = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    const singleComment = comment[0]; // Extract the first element
    ...
    // TypeScript now knows singleComment is defined
